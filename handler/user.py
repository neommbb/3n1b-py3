...
import uuid
import hashlib
import image    # Image change to image
import time
from io import StringIO     # in py3,StringIO is contained in io
import json
import re
import urllib.request       # urllib2 now is sepreted to urlib.request and urllib.error
import urllib.error
import urllib
import tornado.web
import lib.jsonp

from base64 import *        # base changed to base64
...
class HomeHandler(urllib.request.BaseHandler):  # before is "baseHandler",now is"urllib.request.BaseHandler"
    @tornado.web.authenticated
    def get(self):
        user_email = self.current_user['email']
        self.write(user_email)

class SettingHandler(urllib.request.BaseHandler):
    @tornado.web.authenticated
...
class SettingAvatarHandler(urllib.request.BaseHandler):
    @tornado.web.authenticated
    def get(self, template_variables = {}):
        user_info = self.get_current_user()
        template_variables["user_info"] = user_info
...
class SettingAvatarFromGravatarHandler(urllib.request.BaseHandler):
    @tornado.web.authenticated
    def get(self, template_variables = {}):
        user_info = self.current_user
        user_id = user_info["uid"]
...
class SettingPasswordHandler(urllib.request.BaseHandler):
    @tornado.web.authenticated
    def get(self, template_variables = {}):
        user_info = self.get_current_user()
        template_variables["wallpaper"] = self.get_wallpaper()
...
class ForgotPasswordHandler(urllib.request.BaseHandler):
    def get(self, template_variables = {}):
        do_logout(self)
        template_variables["wallpaper"] = self.get_wallpaper()
...
class LoginHandler(urllib.request.BaseHandler):
    def get(self, template_variables = {}):
        do_logout(self)
        template_variables["wallpaper"] = self.get_wallpaper()
    ...
    def post(self, template_variables = {}):
        ...
        if(user_info):
            ...
            print(redirect_path)    # different way to "print" : "print redirect_path" --> "print(redirect_path)"
            redirect_path = redirect_path.replace('1001', str(user_college.id))
            print(redirect_path)    # same way as the upper line
...
class LogoutHandler(urllib.request.BaseHandler):
    def get(self):
        do_logout(self)
        # redirect
        self.redirect(self.get_argument("next", "/"))

class RegisterHandler(urllib.request.BaseHandler):
    def get(self, template_variables = {}):
        do_logout(self)
        template_variables["wallpaper"] = self.get_wallpaper()
...
class RegisterCollegeHandler(urllib.request.BaseHandler):
    def get(self, template_variables = {}):
        user_info = self.current_user
        template_variables["user_info"] = user_info
        if(user_info):
...
class SetCollegeHandler(urllib.request.BaseHandler):
    def get(self, college = None, template_variables = {}):
        ...
        print(collegename)      # "pring collegename" changed to print(collegename)
