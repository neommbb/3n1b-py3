...
class IndexHandler(urllib.request.BaseHandler):
...
class NodeTopicsHandler(urllib.request.BaseHandler):
...
class CollegeTopicsHandler(urllib.request.BaseHandler):
...
class ViewHandler(urllib.request.BaseHandler):
...
class CreateHandler(urllib.request.BaseHandler):
    @tornado.web.authenticated
    ...
    def post(self, college_node = None, template_variables = {}):
        print("CreateHandler:post")     # print()
        template_variables = {}
...
class EditHandler(urllib.request.BaseHandler):
    @tornado.web.authenticated
    def get(self, topic_id, template_variables = {}):
        user_info = self.current_user
        template_variables["user_info"] = user_info
...
class ProfileHandler(urllib.request.BaseHandler):
...
class VoteHandler(urllib.request.BaseHandler):
    def get(self, template_variables = {}):
...
class UserTopicsHandler(urllib.request.BaseHandler):
    def get(self, user, template_variables = {}):
        if(re.match(r'^\d+$', user)):
            user_info = self.user_model.get_user_by_uid(user)
        else:
...
class UserRepliesHandler(urllib.request.BaseHandler):
    def get(self, user, template_variables = {}):
        if(re.match(r'^\d+$', user)):
            user_info = self.user_model.get_user_by_uid(user)
        else:
...
class UserFavoritesHandler(urllib.request.BaseHandler):
    def get(self, user, template_variables = {}):
        if(re.match(r'^\d+$', user)):
            user_info = self.user_model.get_user_by_uid(user)
        else:
...
class ReplyEditHandler(urllib.request.BaseHandler):
    @tornado.web.authenticated
...
class FavoriteHandler(urllib.request.BaseHandler):
    def get(self, template_variables = {}):
        topic_id = int(self.get_argument("topic_id"))
        topic_info = self.topic_model.get_topic_by_topic_id(topic_id)
...
class MembersHandler(urllib.request.BaseHandler):
    def get(self, template_variables = {}):
        user_info = self.current_user
        template_variables["user_info"] = user_info
        if(user_info):
...
class NodesHandler(urllib.request.BaseHandler):
    def get(self, college_node = None, template_variables = {}):
        print("NodesHandler:get")       # print()
        user_info = self.current_user
        template_variables["user_info"] = user_info
        if(user_info):
            template_variables["user_info"]["counter"] = {
...
class CollegesHandler(urllib.request.BaseHandler):
    def get(self, college_node = None, template_variables = {}):
        user_info = self.current_user
        template_variables["user_info"] = user_info
        if(user_info):
...
class FollowNodeHandler(urllib.request.BaseHandler):
...
class FollowUserHandler(urllib.request.BaseHandler):
